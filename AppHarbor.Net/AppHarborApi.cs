...
namespace AppHarbor
{
    public class AppHarborApi
    {
        ...
        private bool ExecuteDeploy(RestRequest request)
        {
            var response = _Client.Execute(request);
            if (response == null)
                return false;
        ...
        public bool Deploy(string applicationID, long ID)
        {
            var request = new RestRequest(Method.POST);
            request.Resource = "applications/{applicationID}/builds/{ID}/deploy";
            request.AddParameter("applicationID", applicationID, ParameterType.UrlSegment);
            request.AddParameter("ID", ID, ParameterType.UrlSegment);
            return ExecuteDeploy(request);
        }
        #endregion
